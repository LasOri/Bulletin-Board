...
public struct App {
    ...
    private static func mountUI() {
        ...
        _ = appStore.subscribe { _ in
            ...
            _ = try? SafeJSGlobal.global?.queueMicrotask.function?(JSClosure { _ in
            ...
            saveTimerId = try? SafeJSGlobal.global?.throwing.setTimeout?(JSClosure { _ in
                Task {
                    let articles = appStore.getState().articles.articles
    ...
    private static func saveScrollPosition() {
        ...
        scrollSaveTimerId = try? SafeJSGlobal.global?.throwing.setTimeout?(JSClosure { _ -> JSValue in
            Task {
    ...
    private static func restoreScrollPosition() {
        ...
        Task {
            do {
                ...
                if saved > 0 {
                    _ = try? SafeJSGlobal.global?.scrollTo.function?(0, saved)
